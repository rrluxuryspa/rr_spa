...
export default function GalleryCarouselSection() {
  ...
  return (
    <section className="py-20 bg-white border-b border-emerald-200/80 relative overflow-hidden">
      ...
      {/* CONTINUOUS SMOOTH MARQUEE CAROUSEL TRACK (PAUSES ON MOUSE HOVER) */}
        ...
        <div className="animate-continuous-scroll flex gap-6 px-4">
          {infiniteImages.map((img, idx) => (
            <div
              ...
              className="w-72 sm:w-80 h-60 sm:h-72 rounded-3xl overflow-hidden border-2 border-emerald-300/80 cursor-pointer shadow-md hover:shadow-2xl hover:border-emerald-500 hover:scale-105 transition-all duration-300 shrink-0 bg-white group"
            >
      ...
      {activeImageModal && (
        ...
        >
          <div className="relative max-w-4xl w-full bg-white border-2 border-emerald-300 rounded-3xl overflow-hidden shadow-2xl p-4">
            <button
              onClick={() => setActiveImageModal(null)}
              className="absolute top-6 right-6 p-2 rounded-full bg-slate-950/70 hover:bg-emerald-600 text-white transition-colors z-10"
